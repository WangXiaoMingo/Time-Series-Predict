...
def fuzzy_fusion(delta,alpha):
    ...
    # x = tf.where(tf.subtract(x1, x2) > 0, x2, alpha*x1+(1-alpha)*x2) # alpha=0.6
    x = tf.where(tf.abs(tf.subtract(x1, x2)) < delta, tf.minimum(x1, x2), alpha*x1+(1-alpha)*x2)
    # 回归层
    x = Dense(units=64)(x)
...
for i in range(0,11):
    algorithm = f'fuzzy_fusion_alpha_{i*0.1}'
    print(f'======================{algorithm}=========================')
    a = time.time()
    model = fuzzy_fusion(delta=0.5,alpha=i*0.1)   #0.00189
    ...
    # 保存结果
    test_result['time'] = b
    test_result.index = [f'alpha={i*0.1}']
