...
def fuzzy_fusion(delta,alpha):
    ...
    # x = tf.where(tf.subtract(x1,x2)>0,x2,x1)                                # 0.00218
    # x = tf.where(tf.subtract(x1, x2) > 0, x2, tf.add(x1, x2) / 2)           # 0.00211
    # x = tf.where(tf.subtract(x1, x2) > 0, x2, alpha*x1+(1-alpha)*x2)        # alpha=0.6, 0.00233; alpha=0.5,0.00213;  alpha=0.6, 0.00233
    x = tf.where(tf.abs(tf.subtract(x1, x2)) < delta, tf.minimum(x1, x2), alpha*x1+(1-alpha)*x2) # delta=alpha=0.6, 0.00207,0.00199
    # 回归层
    x = Dense(units=64)(x)
...
print("---加载数据完成------------------------")
# 数据形状：X_train:(44610, 118)，X_test:(14400, 118),Y_train:(44610, 1),Y_test:(14400, 1)
...
'''=============3. 训练模型======================='''
algorithm = 'fuzzy_fusion_con'
print(f'======================{algorithm}=========================')
# 多次训练取均值，以评价模型性能，均值和标准差
for i in range(1):
    a = time.time()
    model = fuzzy_fusion(delta=0.6,alpha=0.6) # alpha=0.6 # 0.00199
    ...
    optimer = Adam(lr=0.01) #0.00005->0.026;  0.0005-> 0.0079; 0.001-> 0.00614; 0.005-> 0.00307; 0.01-> 0.00229; 0.05->0.00414;0.1->0.02501
